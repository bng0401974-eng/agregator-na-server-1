...
class DashboardSettingsController extends DashboardController
{
    ...
    public function adsPOST()
    {
        ...
        $backgroundImage = trim(sp_strip_tags($req->post('site_background_image_url')));
        $backgroundImageLink = trim(sp_strip_tags($req->post('site_background_image_link')));

        if (empty($backgroundImage) && !empty($_FILES['site_background_image']['name'])) {
            $uploadDir = uploadspath('backgrounds');
            if (!is_dir($uploadDir)) {
                mkdir($uploadDir, 0755, true);
            }

            $storage = new FileSystem($uploadDir, true);
            $file = new File('site_background_image', $storage);
            $fileName = 'site-bg-' . uniqid();

            $file->setName($fileName);
            $file->addValidations([
                new Extension(['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg']),
                new Size(ini_get('upload_max_filesize') ?: '2M')
            ]);

            try {
                $file->upload();
                $backgroundImage = uploads_uri('backgrounds/' . $fileName . '.' . $file->getExtension());
            } catch (\Exception $e) {
                flash('settings-warning', sprintf(__('Failed to upload background image. Reason: %s'), join('<br>', $file->getErrors())));
            }
        }

        $data['site_background_image'] = $backgroundImage;
        $data['site_background_image_link'] = $backgroundImageLink;
