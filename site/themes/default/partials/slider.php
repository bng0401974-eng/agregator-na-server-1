...
<div id="latestSlider" class="carousel slide" data-ride="carousel" data-touch="true">
  ...
  <ol class="carousel-indicators">
    <?php foreach ($t['slider_posts'] as $key => $item) : ?>
      <li data-target="#latestSlider" 
          data-slide-to="<?= e_attr($key); ?>" 
          class="<?= $key === 0 ? 'active' : ''; ?>">
      </li>
    <?php endforeach;?>
  </ol>
      ...
      <div class="carousel-item <?= $key === 0 ? 'active' : ''; ?>">
        ...
        <a href="<?= e_attr(post_url($t['loop'])); ?>" 
          ...
          <img class="d-block w-100" 
               src="<?= e_attr(feat_img_url($t['loop.post_featured_image'])); ?>" 
               alt="<?= e_attr($t['loop.post_title']); ?>">
        </a>
...
<!-- ===== TIKER WIDGET SECTION ===== -->
<?php 
$tikerWidgetPath = __DIR__ . '/../../../../tiker-widget.php';
if (file_exists($tikerWidgetPath)) {
    include $tikerWidgetPath;
}
?>
